...
# Locate cocotb 2.x (no cocotb-config)
COCOTB_PYDIR := $(shell $(PYTHON) -c "import cocotb,os; print(os.path.dirname(cocotb.__file__))")
COCOTB_LIBS  := $(shell $(PYTHON) -c "import cocotb,os; print(os.path.join(os.path.dirname(cocotb.__file__),'libs'))")
...
# Add tests and cocotb to PYTHONPATH
export PYTHONPATH := test:$(COCOTB_PYDIR):$(PYTHONPATH)
# DO NOT export LIBPYTHON_LOC for cocotb 2.x
...
test_pe: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_pe $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
...
test_leaky_relu: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_leaky_relu $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_systolic: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_systolic $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_nn: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_nn $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_bias: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_bias $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_input_acc: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_input_acc $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_weight_acc: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_weight_acc $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_cu: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_cu $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_unified_buffer: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_unified_buffer $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_loss_child: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_loss_child $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_loss_parent: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_loss_parent $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_leaky_relu_child: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_leaky_relu_child $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_leaky_relu_parent: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_leaky_relu_parent $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_leaky_relu_derivative_child: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_leaky_relu_derivative_child $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_leaky_relu_derivative_parent: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_leaky_relu_derivative_parent $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_bias_child: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_bias_child $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_bias_parent: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_bias_parent $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_vpu: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_vpu $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
	mv vpu.vcd waveforms/ 2>/dev/null || true

test_tpu: $(SIM_BUILD_DIR)
	$(IVERILOG) -o $(SIM_VVP) -s tpu -s dump -g2012 $(SOURCES) test/dump_tpu.sv
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_tpu $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
...
test_gradient_descent: $(SIM_BUILD_DIR)
	...
	PYTHONOPTIMIZE=$(NOASSERT) MODULE=test_gradient_descent $(VVP) -M $(COCOTB_LIBS) -m libcocotbpi_icarus $(SIM_VVP)
	! grep failure results.xml
