...
1. It is recommended to take a look and familiarize yourself with the tutorial at the following link in order to better understand the general design flow for the Ultra96-V2 board, which is the board we will be using to implement CornellTPU for now: https://github.com/yxd97/ultra96-example/tree/master 
...
2. First, make sure your RTL design has been fully completed and tested (in simulation), as this is of course crucial for it to work within a larger design
3. Once you are sure it is functioning correctly, add it to a Vivado project if you have not already done so. I also recommend using Vivado via the GUI, which will require you to connect via remote desktop.
...
5. Now, we need to modify the generated Verilog HDL code for the IP
   ...
   - The file called [ip_name].v is the top level file of the ip - within this module, you must instantiate your RTL design, connect its inputs and outputs to the AXI interface module, and add any additional ports/parameters/additional logic you want
...
6. Finally, your IP should be ready to be added to the block diagram - from this point on everything is very similar to the HLS tutorial linked above. Add, configure, and connect all relevant IPs (zynq ultrascale mpsoc, clock wizard, your custom ip, etc.), validate the block diagram, and generate the bitstream.
7. As in the aove tutorial, you must also write a host program to run on the PS, and then follow a similar series of steps for loading and testing the design on hardware.
   - The contents of host.py will be heavily dependent on your specific custom IP
   - The pynq documentation can be very helpful: https://pynq.readthedocs.io/en/v2.6.1/
